...
# --------------------------------------------------------------------------------------------
# (현수) 찾아보니 NC 의미가 Nominal & Continuous라네요. Null 미삭제 시에는 사용 가능할것도 같습니다.참고차..^^ 
# --------------------------------------------------------------------------------------------
# 
...
# ---------------- rf_params 일부 수정했습니다. estimators와 depth를 바꾸었습니다 -----------
rf_params = [{'clf__n_estimators': [10, 50, 100, 300], 'clf__max_depth' : [3, 4, 5, 10, 50]}]
...
# --------------------------------------------------------------------------------------------
# 위에꺼 더 단순한 코드로 만들 수 있을 거 같아서 해봤습니다
...
def fit_cv(X_train, y_train, X_test, y_test, scaler='RB', scoring='recall', conf_m=True, view_scores=True, draw_cv=True, n_jobs=None, **kwargs):
    """
    GridSearchCV. 
...
# --------------------------------------------------------------------------------------------        
# 다른 곳은 아직 미확인이긴 한데, n_jobs=-1시 속도 3배 이상 향상. 안 쓴 이유가 있으신지?
# --------------------------------------------------------------------------------------------        
